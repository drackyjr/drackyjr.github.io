...
The challenge contains multiple decoys:

- A backup agent running before the incident
- A printer-health WMI filter and consumer
- A zero-byte `asteria-agent.exe`
- Browser history that makes generic WMI research look suspicious
- Normal Run-key content

The solver must correlate multiple artifact types instead of trusting a single
indicator.
...
- Windows WMI Activity Event ID 5861
- `WmiPrvSE.exe` spawning PowerShell
- Permanent WMI subscription objects in `root\subscription`
- Custom WMI classes carrying opaque or encoded data
- Hourly or periodic execution with no Scheduled Task present
- Service-account network logons immediately preceding persistence creation
...
- remote-control code
- persistence installation commands

Everything is a static forensic artifact for CTF/DFIR training.
